...
def get_video_info(video_url, max_retries=3):
    # Validate Facebook URL only
    if not ('facebook.com' in video_url or 'fb.watch' in video_url):
        raise ValueError("Only Facebook video URLs are supported")
        ...
        try:
            ...
            if "Cannot parse data" in error_msg and attempt < max_retries - 1:
                ...
                with st.expander("🔧 Troubleshooting Tips"):
                    ...
                    - **Wait a few minutes** - Facebook may temporarily block requests
                    - **Check if the video is public** - Private/restricted videos cannot be downloaded
                    - **Use full URL**: Copy the full Facebook URL, not shortened links
                    """)
            else:
...
if url and url.strip():
    url_lower = url.lower()
    if not ('facebook.com' in url_lower or 'fb.watch' in url_lower):
        st.error("❌ Only Facebook video URLs are supported. Please enter a valid Facebook video or reel URL.")
        ...
        if info.get('thumbnail'):
            try:
                ...
                if preview_url:
                    ...
                    <p style="text-align:center; color:#ccc; font-size:0.9em; margin-top:6px;">Facebook Video Thumbnail</p>
                    """, unsafe_allow_html=True)
            except Exception:
