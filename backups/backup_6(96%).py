...
if url and url.strip() and (not st.session_state.video_info or st.session_state.video_info.get('webpage_url') != url.strip()):
    with st.spinner("Getting video information..."):
        ...
        if video_info:
            ...
            # Sắp xếp: format có audio trước, sau đó theo height giảm dần
            video_formats.sort(key=lambda x: (x.get('has_audio', False), x['height'], x.get('filesize', 0)), reverse=True)
...
if st.session_state.video_info:
    ...
    with col_right:
        # Tự động chọn chất lượng cao nhất với audio
        import shutil
        ffmpeg_available = bool(shutil.which("ffmpeg"))
        
        if st.session_state.formats:
            # Lấy format chất lượng cao nhất (format đầu tiên sau khi sắp xếp)
            best_format = st.session_state.formats[0]
            st.success(f"🎯 Auto-selected: **{best_format['label']}**")
            
            if best_format.get('has_audio'):
                # Format đã có audio, dùng trực tiếp
                fmt = best_format['format_id']
                st.info("✅ Using format with built-in audio")
            elif ffmpeg_available:
                # Merge video chất lượng cao với audio tốt nhất
                fmt = f"{best_format['format_id']}+bestaudio/best"
                st.info("🔧 Merging best video + audio (FFmpeg available)")
            else:
                # Fallback: tìm format tương đương có audio
                fmt = f"best[height<={best_format['height']}][acodec!=none]/best[acodec!=none]/best"
                st.warning("⚠️ FFmpeg not available - using compatible format")
        else:
            # Fallback khi không có formats
    ...
    # Fallback cho môi trường chưa load video info - luôn tải chất lượng cao nhất với audio
    import shutil
    ffmpeg_available = bool(shutil.which("ffmpeg"))
    if ffmpeg_available:
        fmt = "bv*+ba/b"  # Best video + best audio merge
    else:
        fmt = "best[acodec!=none]/best"  # Best format với audio
    outtmpl = "downloads/%(title).80s-%(id)s.%(ext)s"
